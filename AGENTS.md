...
6. **クラウドサービス不使用**: 会場LAN内で完結させる。外部のシグナリングサーバーやクラウドサービスへの依存を追加しない
   - **唯一の例外(#23、本番デモ時のみ)**: 飛び入り参加者の証明書警告を消すため、公開DNS(Cloudflare)での名前解決だけ外部に依存する。ネットに出るのは「ドメイン名 → 会場のLAN IP」の問い合わせだけで、**モデル配布も推論のテンソルも会場LANから出ない**(前提2は維持)。開発中は従来通り mkcert で完結する。詳細は `apps/server/README.md`「本番デモの証明書」
