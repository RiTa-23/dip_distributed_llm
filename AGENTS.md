...
6. **クラウドサービス不使用**: 会場LAN内で完結させる。外部のシグナリングサーバーやクラウドサービスへの依存を追加しない
   - **唯一の例外(#23、本番デモ時のみ)**: 飛び入り参加者の証明書警告を消すため、公開DNSに依存する。外に出るのは(a)参加者端末の名前解決「ドメイン名 → 会場のLAN IP」、(b)発表者PCからCloudflare APIへのAレコード検索・更新(`bun run dns`)、(c)発表者PCからの証明書取得(`acme.sh`、事前)の3つだけ。**参加者のデータプレーン(モデル配布・推論のテンソル)と `/ws` の制御メッセージは会場LANから出ない**(前提2は維持)
