...
### 発表者が編成を白紙に戻す(全員解除、#114)

発表者画面のPEERS見出しの右に「全員解除」ボタンがあります。押すとHonoへ `dismiss_peers` を送り、Honoが全peerをロスターから降ろして `peers_dismissed` を配ります(契約は `api-contract.md` の10・11)。

- **2段押しです。** 1回目は実行せず「もう一度押すと解除」に変わり、4秒で元に戻ります。参加者全員を巻き込むうえ、戻すには全員に参加し直してもらうしかないので、1クリックでは起きないようにしてあります
- **参加者は参加前の画面へ戻ります。**「発表者が編成を解除しました」を出し、表示名は残したまま「参加する」で1手で戻れます。**自動では参加し直しません** — 勝手に戻ると解除になりません
- 発表者側は `waiting` に戻ります。`reorganizing` にはしません。次の `generation_start` は誰かが参加し直すまで来ないので、「まもなく再開します」が嘘になるためです
- WebRTCの後片付けは `generation_aborted` と同じ経路です(`useWebrtcSignaling` が `teardown()` を呼び、`rtc.generation` が0に戻って Runtime も畳まれます)
- ⚠️ **サーバがWebSocketを閉じるだけでは解除になりません。** `useHonoSocket` が250ms〜4秒で自動的に繋ぎ直し、`hello` から入り直して同じ顔ぶれが組まれます。専用のメッセージが要るのはこのためです
...
| `--c-danger` | `#D25447` | ピア状態ドットの error、全員解除ボタンの実行直前 |
...
`--c-danger` の状態ドット以外での使用は、いまのところ発表者画面の「全員解除」ボタンだけです(`#114`)。**2段押しの2手目でだけ**色が付き、平常時は他のボタンと同じ見た目にしてあります。常に赤いと参加者一覧そのものが警告に見えてしまうためです。破壊的な操作を増やすときは、この形(平常時は無色・実行直前だけ `--c-danger`)に揃えてください。
...
| `views/RequesterView.*` | チャット、ピア一覧、モデルの進捗、全員解除 |
